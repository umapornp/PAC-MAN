...
class ProgressBarLogger(Callback):
    ...
    def on_evaluate_end(self):
        ...
        if not self.do_train:
            LOG.info("Evaluation results: " + self._log_loss(["val_loss"]) + self._log_metrics())
    ...
    def on_epoch_end(self, epoch):
        ...
        self.pbar.close()
        LOG.info("Training results: " + self._log_loss(["train_loss", "val_loss"]) + self._log_metrics())
    ...
    def _log_loss(self, loss=[]):
        ...
        msg = ""
        for name in loss:
            msg += f"{name}: {self.trainer.losses[name].get():0.4f} | "
        return msg
