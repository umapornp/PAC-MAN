...
from pacbert.utils.logging import TrainLogger, LOG
from pacbert.utils.callbacks import CallbackCaller
from pacbert.utils.metrics import MetricCaller
...
class Trainer:
    ...
    def _evaluate_step(self, inputs, labels):
        ...
        with torch.no_grad():
            logits = self._predict_step(inputs) # Evaluate.
    ...
    def _predict_step(self, inputs, mode='seq'):
        """ Prediction step.
        
        Args:
            inputs: Dictionary of user_ids, text_ids, and tag_ids.
            mode: Prediction mode (`seq` and `all`).
        """
        if mode == 'seq':
            return self._predict_seq(inputs)
        elif mode == 'all':
            return self._predict_all(inputs)


    def _predict_all(self, inputs):
        """ Predict all masked inputs.
        
        Args:
            inputs: Dictionary of user_ids, text_ids, and tag_ids.
        
        Returns:
            Logits of model output.

        """
        return self.model(**inputs)[0]


    def _predict_seq(self, inputs):
        """ Predict masked inputs from previous predictions.
        
        Args:
            inputs: Dictionary of user_ids, text_ids, and tag_ids.
        
        Returns:
            Logits of model outputs.
        """
        mask_tok = TagTokenizer().vocab["[MASK]"]
        mask_idx = inputs["tag_ids"] == mask_tok
        mask_len = mask_idx.sum(-1).tolist()

        # Get Y-indices of mask tokens. 
        y = mask_idx.nonzero(as_tuple=True)[1].split(mask_len)
        y = pad_sequence(y, padding_value=-1)

        # Get X-indices of mask tokens.
        num_mask, bs = y.shape
        x = torch.arange(bs).unsqueeze(0).repeat(num_mask, 1)

        logits = None
        for t in range(num_mask):
            msk = y[t] != -1 # Ignore padding labels.
            idx_tags = (x[t][msk], y[t][msk]) # Get mask index.
            ...
            logits[idx_tags] = outputs[idx_tags]
            inputs["tag_ids"][idx_tags] = outputs[idx_tags].argmax(-1)
